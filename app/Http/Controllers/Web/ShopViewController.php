...
{
    //for seller Shop
    public function seller_shop(Request $request, $id,$seller)

    {
       // dd($id);
        $theme_name = theme_root_path();
        // $url = route('shopView', ['id' => $id->id, 'shopname' => $shopname]);
...
// public function seller_shop(Request $request, $id, $seller)
// {
//     // Fetch the shop using the ID (already done in the route binding)
//     $shop = Shop::find($id);

//     // If the shop is not found, handle accordingly (e.g., show a 404 page)
//     if (!$shop) {
//         abort(404);
//     }

//     // Accessing the shop name (slug)
//     $shopnameFromModel = $shop->name;

//     // Check if the provided shopname matches the one from the model
//     $shopname = $request->input('seller');
//     if (!$shopname || strtolower($shopname) !== strtolower($shopnameFromModel)) {
//         // Set the correct shop name in lowercase
//         $shopname = strtolower($shopnameFromModel);

//         // Check if the current URL is already correct before redirecting
//         $currentUrl = route('shopView', ['id' => $id, 'seller' => $shopname]);
//         if ($request->url() !== $currentUrl) {
//             // Redirect to the correct URL with the updated shopname
...
//     // Updated URL generation with both id and shopname
//     // $url = route('shopView', ['id' => $id, 'seller' => $shopname]);

//     // Use a switch statement instead of match for better readability
//     switch ($theme_name) {
//         case 'default':
//             return self::default_theme($request, $id);
//         case 'theme_aster':
//             return self::theme_aster($request, $id);
//         case 'theme_fashion':
//             return self::theme_fashion($request, $id);
//         case 'theme_all_purpose':
//             return self::theme_all_purpose($request, $id);
//         default:
//             // Handle unknown theme name, maybe redirect to a default theme
//             return redirect()->route('defaultTheme');
//     }
// }
