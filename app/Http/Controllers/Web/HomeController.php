...
class HomeController extends Controller
{
    ...
    public function theme_aster()
    {
        ...
        $category_slider = array_chunk($final_category, 2);
        ...
        if (auth('customer')->check()) {
            ...
            if ($orders) {
                ...
                ])->active()->inRandomOrder()->take(6)->get();
            }
        } else {
            $just_for_you = $this->product->with([
                'wish_list'=>function($query){
        ...
        $banners = $this->banner->whereIn('banner_type', $banner_list)->where(['published'=> 1, 'theme'=>$theme_name])->orderBy('id', 'asc')->latest('created_at')->get();
        ...
        $footer_banner = $footer_banner ? array_slice($footer_banner, 0, 5):[];
        ...
        $order_again = $user != 'offline' ?
            $this->order->with('details.product')->where(['order_status' => 'delivered', 'customer_id' => $user->id])->latest()->take(1)->get()
            : [];
    ...
    public function theme_all_purpose(){
        ...
        $footer_banner = $this->banner->where('banner_type', 'Footer Banner')->where('published', 1)->latest()->get();
