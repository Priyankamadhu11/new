...
@php($decimal_point_settings = \App\CPU\Helpers::get_business_settings('decimal_point_settings'))
    ...
    <div class="container py-3" dir="{{Session::get('direction')}}">
        <div class="search-page-header">
            ...
            <form id="search-form" class="d-none d-lg-block" action="{{ route('products'),'name' => $data->name }}" method="GET">
                                ...
                                @foreach($categories as $category)
                                    <div class="menu--caret-accordion">
                                        <div class="card-header flex-between">
                                            @php
    $categoryNameWithPlus = str_replace(' ', '+', strtolower($category['name']));
@endphp
                                            <div>
                                                <label class="for-hover-lable cursor-pointer" onclick="location.href='{{route('products',['id'=> $categoryNameWithPlus,'data_from'=>'category','name'=>$category['name'],'sub'=>$categoty->childes->name,'page'=>1])}}'">
                                                    {{$category['name']}}
                                                </label>
                                            </div>
                                            ...
                                            @foreach($category->childes as $child)
                                                <div class="menu--caret-accordion">
                                                    <div class="for-hover-lable card-header flex-between">
                                                        ...
                                                        <div>
                                                            <label class="cursor-pointer" onclick="location.href='{{route('products',['id'=> $child['id'],'data_from'=>'category','name'=>$category['name'],'sub'=> $child['name'],'page'=>1])}}'">
                                                                {{$child['name']}}
                                                            </label>
                                                        </div>
                                                        <div class="px-2 cursor-pointer menu--caret">
                                                    ...
                                                    <div class="card-body p-0 {{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}}"
                                                        ...
                                                        @foreach($child->childes as $ch)
                                                            <div class="card-header">
                                                                <label class="for-hover-lable d-block cursor-pointer text-left" onclick="location.href='{{route('products',['id'=> $ch['id'],'data_from'=>'category','name'=>$category['name'],'sub'=> $ch['name'],'page'=>1])}}'">
                                                                    {{$ch['name']}}
                                                                </label>
                                                            </div>
                                                        @endforeach
