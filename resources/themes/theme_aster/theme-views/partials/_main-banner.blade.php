                            ...
                            <ul class="dropdown-menu dropdown-menu--static"
                                ...
                                @foreach($categories as $key=>$category)
                                    <li class="{{ $category->childes->count() > 0 ? 'menu-item-has-children' : '' }}">
                                        <a href="javascript:" onclick="location.href='{{route('products',['id'=> $category['id'],'name'=>$category['name'],'page'=>1])}}'">
                                            {{$category['name']}}
                                        </a>
                                            ...
                                            <ul class="sub-menu">
                                                @foreach($category['childes'] as $subCategory)
                                                    <li class="{{ $subCategory->childes->count()>0 ? 'menu-item-has-children' : '' }}">
                                                        <a href="javascript:" onclick="location.href='{{route('products',['id'=> $subCategory['id'],'data_from'=>'category','name'=>$category['name'],'page'=>1])}}'">
                                                            {{$subCategory['name']}}
                                                        </a>
                                                        @if($subCategory->childes->count()>0)
                                                            <ul class="sub-menu">
                                                                @foreach($subCategory['childes'] as $subSubCategory)
                                                                    <li>
                                                                        <a href="{{route('products',['id'=> $subSubCategory['id'],'data_from'=>'category','name'=>$category['name'],'page'=>1])}}">
                                                                            {{$subSubCategory['name']}}
                                                                        </a>
                                                                    </li>
                                                                @endforeach
                                                ...
                                                <div class="d-flex flex-column gap-2 p-3">
                                                    ...
                                                    <h6 class="fs-12">
                                                        ...
                                                        <span class="text-uppercase ">
                                                            ...
                                                            @else
                                                               <a class="shop-name" onclick="location.href='{{ isset($coupon->seller->shop) ? route('shopView', ['id' => $coupon->seller->shop['id'], 'seller' => $coupon->seller->shop['name']]) : 'javascript:' }}'">
