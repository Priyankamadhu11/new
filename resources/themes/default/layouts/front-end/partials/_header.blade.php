                    ...
                    <ul class="navbar-nav mega-nav1 pr-md-2 pl-md-2 d-block d-xl-none"><!--mobile-->
                        <li class="nav-item dropdown d-md-none">
                            ...
                            <ul class="dropdown-menu __dropdown-menu-2"
                               ...
                               @php($categoryNameWithp = str_replace(' ', '+', strtolower($category['name'])))
                                    <li class="dropdown">

                                        <a <?php if ($category->childes->count() > 0) echo ""?>
                                        href="{{route('products',['id'=> $category['id'],'data_from'=>'category','name' =>$categoryNameWithp,'page'=>1])}}">
                                        ...
                                        @if($category->childes->count()>0)
                                            <ul class="dropdown-menu"
                                                ...
                                                @foreach($category['childes'] as $subCategory)
                                                    <li class="dropdown">
                                                        <a  href="{{route('products',['id'=> $subCategory['id'],'data_from'=>'category','name'=>$categoryNameWithPlus,'page'=>1])}}">
                                                        ...
                                                        <a style="font-family:  sans-serif !important;font-size: 1rem;
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                @foreach($subCategory['childes'] as $subSubCategory)
                                                                    <li>
                                                                        <a class="dropdown-item"
                                                                           href="{{route('products',['id'=> $subSubCategory['id'],'data_from'=>'category','name'=>$categoryNameWithPlus,'page'=>1])}}">{{$subSubCategory['name']}}</a>
                                                                    </li>
                                                                @endforeach
                                                            </ul>
                                                        @endif
                            ...
                            <ul class="dropdown-menu __dropdown-menu-sizing dropdown-menu-{{Session::get('direction') === "rtl" ? 'right' : 'left'}} scroll-bar"
                                ...
                                @foreach(\App\CPU\BrandManager::get_active_brands() as $brand)
                                    <li class="__inline-17">
                                        @php($brandNameWithPlus = str_replace(' ', '+', strtolower( $brand['name'])))
                                        <div>
                                            <a class="dropdown-item"
                                               href="{{route('products',['id'=> $brand['id'],'data_from'=>'brand','name'=>brandNameWithPlus',page'=>1])}}">
                                                {{$brand['name']}}
                                            </a>
                                        </div>
                ...
                <div class="category-menu-wrap">
                    <ul class="category-menu">
                        @foreach ($categories as $key=>$category)
                            <li>
                                @php($categoryNameWithPlus=str_ireplace(' ','+',$category['name']))
                                <a href="{{route('products',['id'=> $category['id'],'data_from'=>'category','name'=>$categoryNameWithPlus,'page'=>1])}}">{{$category->name}}</a>
                                @if ($category->childes->count() > 0)
                                    <div class="mega_menu z-2">
                                        @foreach ($category->childes as $key=>$sub_category)
                                            <div class="mega_menu_inner">
                                            @php($subcategoryNameWithPlus=str_ireplace(' ','+',$sub_category['name']))

                                                <h6><a href="{{route('products',['id'=> $sub_category['id'],'data_from'=>'category','name'=>$subcategoryNameWithPlus,'page'=>1])}}">{{$sub_category->name}}</a></h6>
                                                @if ($sub_category->childes->count() >0)
                                                    @foreach ($sub_category->childes as $sub_sub_category)
                                                        <div><a href="{{route('products',['id'=> $sub_sub_category['id'],'data_from'=>'category','name'=>$subcategoryNameWithPlus,'page'=>1])}}">{{$sub_sub_category->name}}</a></div>
                                                    @endforeach
                                                @endif
                                            </div>
                                        @endforeach
