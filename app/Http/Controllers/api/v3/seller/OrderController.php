...
class OrderController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $orders = Order::with('offline_payments')->with(['customer','shipping', 'delivery_man'])
        ...
        $orders->map(function ($data) {
            if(isset($data['offline_payments'])){
                $data['offline_payments']->payment_info = json_decode($data->offline_payments->payment_info);
            }
    ...
    public function details(Request $request, $id)
    {
        ...
        $details = OrderDetail::with('order.customer','order.delivery_man','verification_images')->where(['seller_id' => $seller['id'], 'order_id' => $id])->get();
    ...
    public function assign_third_party_delivery(Request $request)
    {
        ...
        $order->third_party_delivery_tracking_url = $request->third_party_delivery_tracking_url;
    ...
    public function address_update(Request $request){
        ...
        if ($request->address_type == 'shipping') {
            $order->shipping_address_data = json_encode($shipping_address_data);
        } elseif ($request->address_type == 'billing') {
            $order->billing_address_data = json_encode($billing_address_data);
        }
        $order->save();
