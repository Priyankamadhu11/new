...
<div style="width:650px;margin:auto; background-color:white;padding:40px;border-radius: 3px;">
    ...
    <?php
        $seller = \App\Model\Seller::find($seller_id);
        $shop = \App\Model\Shop::find($seller->id);
    ?>

    <!-- resources/views/emails/request_seller_delete.blade.php -->

        <p>Dear Admin,</p>

        <p>A request has been submitted to delete the seller account. Below are the details:</p>

        <ul>
            <li><strong>Seller Name:</strong> {{ $seller->f_name }} {{ $seller->l_name }}</li>
            <li><strong>Seller Email:</strong> {{ $seller->email }}</li>
            <li><strong>Reason for Deletion:</strong> {{ $seller->reason_for_delete }}</li>
        </ul>
