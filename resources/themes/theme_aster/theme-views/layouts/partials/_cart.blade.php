<div class="dropdown">
    <a href="{{route('shop-cart')}}" class="position-relative" data-bs-toggle="dropdown" data-bs-auto-close="outside"
        ...
        <i class="bi bi-bag fs-18 text-light"></i>
            ...
            <div class="media gap-3">
                <div class="avatar avatar-xxl position-relative overflow-hidden rounded">
                    ...
                    onerror="this.src='{{theme_asset('assets/img/image-place-holder.png')}}'"
                     loading="lazy" alt="Product" class="img-fit dark-support rounded img-fluid overflow-hidden {{ $product->status == 0?'blur-section':'' }}">
                    @if ($product->status == 0)
                    ...
                    <div class="d-flex gap-3 justify-content-between align-items-end">
                        ...
                        <div class="quantity">
                            ...
                            @else
                                ...
                                <input type="text" class="quantity__qty cart_quantity_of_{{ $cartItem['id'] }}" data-min="{{ $product ? $product->minimum_order_qty : 1 }}" value="0"
                                onchange="updateCartQuantity('{{ $cartItem['id'] }}','{{ $cartItem['product_id'] }}', '0')">
                            @endif
                        </div>
                    </div>
