...
@section('title', translate('App banner'))
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 text-capitalize">{{ translate('App_banner_form')}}</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <tr id="data-{{$banner->id}}">
                                    ...
                                    <td>
                                        <img class="ratio-4:1" width="80"
                                             ...
                                             src="{{asset('storage/app/public/app-banner')}}/{{$banner['photo']}}">
                                    </td>
                                    <td>{{translate(str_replace('_',' ',$banner->banner_type))}}</td>
                                    <td>
                                        <form action="{{route('admin.app_banner.status')}}" method="post" id="banner_status{{$banner['id']}}_form" class="banner_status_form">
                                            @csrf
                                    ...
                                    <td>
                                        <div class="d-flex gap-10 justify-content-center">
                                            <a class="btn btn-outline--primary btn-sm cursor-pointer edit"
                                               ...
                                               href="{{route('admin.app_banner.edit',[$banner['id']])}}">
                                                <i class="tio-edit"></i>
                                            </a>
...
@push('script')
    ...
    <script>
        ...
        $('.cancel').on('click', function () {
            $('.banner_form').attr('action', "{{route('admin.app_banner.store')}}");
            $('#main-banner').hide();
        });
        ...
        $('.banner_status_form').on('submit', function(event){
            ...
            $.ajax({
                url: "{{route('admin.app_banner.status')}}",
                method: 'POST',
                data: $(this).serialize(),
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.app_banner.delete')}}",
                        method: 'POST',
                        data: {id: id},
